...
- 首页底栏增加 MCP 连接数和版本信息对齐展示（`ROCode v2026.2.27`）
...
- Question 弹窗支持完整键盘交互：`Up/Down`、`Tab/Shift+Tab` 导航，`Space` 选择，`Enter` 提交，`Esc` 拒绝。
- Question 队列会周期同步服务端 `/question`，并通过 `/question/{id}/reply` 与 `/reject` 完成回调闭环。
