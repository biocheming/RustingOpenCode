...
impl LlmProcessor {
    ...
    async fn stream_from_input(
        ...
        let system_hook_outputs = rocode_plugin::trigger_collect(
        ...
        let params_hook_outputs = rocode_plugin::trigger_collect(
        ...
        let header_hook_outputs = rocode_plugin::trigger_collect(
...
fn message_to_model(info: &MessageInfo, parts: &[Part]) -> Option<Message> {
    let role = match info {
        MessageInfo::User { .. } => rocode_provider::Role::User,
        MessageInfo::Assistant { .. } => rocode_provider::Role::Assistant,
    };
...
fn part_to_content_part(part: &Part) -> Option<rocode_provider::ContentPart> {
    match part {
        Part::Text { text, .. } => Some(rocode_provider::ContentPart {
        ...
        Part::File(file_part) => Some(rocode_provider::ContentPart {
